...
def has_request_arg(fn):
    ...
    for name,param in params:
        if name == 'request':
            found = True
            continue
        if found and (str(param.kind)!='VAR_POSITIONAL' and str(param.kind)!='VAR_KEYWORD' and str(param.kind)!='KEYWORD_ONLY'):
...
class RequestHandler(object):
    def __init__(self,app,fn):
        self.app = app
        self.func = fn
        self._requird_kw_args = get_required_kw_args(fn)
    ...
    async def __call__(self,request):
            ...
            if request.math =='POST':
                ...
                if ct.startwith('application/json'):
                    ...
                    if not isinstance(params,dict):
                        return web.HTTPBadRequest(text='json must be dict')
                    kw = params
        ...
        if self._has_request_arg:
            kw['request'] = request
        if self._requird_kw_args:
            for name in self._requird_kw_args:
                if name not in kw:
